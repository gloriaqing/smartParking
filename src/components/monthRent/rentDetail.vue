			  ...
			  if(this.items.length == 0){ // 没有租约，让其重新去物管添加租约
			  	...
			  	console.log("11111111");
					...
					for (var i=0;i<res.data.data.length;i++){
						...
						console.log(res.data.data[i].reg_start_time)
						localStorage.setItem('park_id',res.data.data[i].parklot_id)
						...
						localStorage.setItem('rentTime',res.data.data[i].reg_duration)
