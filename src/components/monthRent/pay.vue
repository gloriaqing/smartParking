...
<template>
	<div>
		...
		<div style="width:80%;margin: 0 auto;border-bottom: 1px solid #bfbfbf;">
			<h5>应付金额</h5>
			<p style="font-size: 22px;font-weight: 600;">￥15.00元</p>
		</div>
		<div class="cont">
				...
				<ul>
					<li v-model="carNumber" style="margin-top: 20px;">
						...
						缴费车辆<span style="margin-left: 20px;color: #000;font-size: 16px;">{{carNumber}}</span></li>
					<li v-model="formatD">
						<img src="../../assets/img/icon/位置.png" style="width: 25px;margin-right: 10px" />
						停车位置<span style="margin-left: 20px;">{{formatD}}</span></li>
					<li>
						<img src="../../assets/img/icon/开始时间.png" style="width: 25px;margin-right: 10px" />
						入场时间<span style="margin-left: 20px;">{{endDate}}</span></li>
					<li v-model='range'>
						<img src="../../assets/img/icon/结束时间.png" style="width: 25px;margin-right: 10px" />
						出场时间<span style="margin-left: 20px;">{{range}}</span></li>
					<li v-model="fee">
						<img src="../../assets/img/icon/钱 (2).png" style="width: 25px;margin-right: 10px" />
						总金额<span style="margin-left: 20px;"> {{fee}}</span></li>
				</ul>
				
		</div>
		<div style="background: #f0f0f0;">
			<button type="button" @click="pay" class="btn btnPay" data-toggle="modal" data-target="#myModal"> 微信支付</button>
...
<script>
	export default {
		...
		created() {
			...
			console.log(this.formatD)
			this.carNumber = localStorage.getItem('carNumber'),
			this.formatD = this.$route.params.deviceId,
			this.range = this.$route.params.getRange,
			this.endDate = this.$route.params.endDate,
			this.userid=localStorage.getItem('userid'),
			this.$http.post('http://api.basecn.cn/cloud/api/park/findFee',{userId:'8a9909325722ee33015722f81d1d0002',carNum:'云AE0E25'}).
			then(res=>{
				console.log(res)
        		this.fee=res.data.data.shouldFee
			})
		},
		mounted() {
			this.jsapiConfig()
...
//				var openid = localStorage.getItem('openid');
				...
				this.$http({
					method: 'post',
					data: {
						"openId": "oQeycv2K9y3tweM2Z2d1_IFcI1Ug",
						"total_fee": this.fee,
						"body": "停车场缴费",
						"detail": "停车场缴费",
						"attach": "无附加数据"
